...
- If a new airline, operator, route endpoint, airport, model, or callsign family appears in a live aircraft-window case, update the relevant speech pack and fallback tables instead of relying on Latin spelling. Add or update `custom_components/aircraft_window/speech_ru.py` tables, known callsign/route fallbacks, and focused tests as applicable.
